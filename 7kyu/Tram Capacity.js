...
// My Solution
function tram(n, exiting, entering) {
  let passengersInside = 0;
  let maxCapacity = 0;

  for (let i = 0; i < n; i++) {
    passengersInside -= exiting[i]; // Exiting passengers first
    passengersInside += entering[i]; // Then entering passengers
    maxCapacity = Math.max(maxCapacity, passengersInside);
  }
