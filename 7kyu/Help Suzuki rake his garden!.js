...
function rakeGarden(garden) {
  // Split the garden string into an array of words
  const gardenArray = garden.split(" ");

  // Loop through the array and replace items that are not "rock" with "gravel"
  for (let i = 0; i < gardenArray.length; i++) {
    if (gardenArray[i] !== "rock") {
      gardenArray[i] = "gravel";
    }
  }

  // Join the array back into a string and return the result
  return gardenArray.join(" ");
}
